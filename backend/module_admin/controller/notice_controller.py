...
@noticeController.get("/list", response_model=PageResponseModel,
                      dependencies=[Depends(CheckUserInterfaceAuth('system:notice:list'))])
async def get_system_notice_list(request: Request,
...
async def add_system_notice(request: Request, add_notice: NoticeModel, query_db: Session = Depends(get_db),
...
async def edit_system_notice(request: Request, edit_notice: NoticeModel, query_db: Session = Depends(get_db),
                             current_user: CurrentUserModel = Depends(LoginService.get_current_user)):
    """
    修改通知
    """
...
@noticeController.get("/{notice_id}", response_model=NoticeModel,
                      dependencies=[Depends(CheckUserInterfaceAuth('system:notice:query'))])
async def query_detail_system_notice(request: Request, notice_id: int, query_db: Session = Depends(get_db)):
    try:
