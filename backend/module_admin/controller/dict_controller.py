...
@dictController.get("/type/list", response_model=PageResponseModel,
                    dependencies=[Depends(CheckUserInterfaceAuth('system:dict:list'))])
async def get_system_dict_type_list(request: Request,
                                    dict_type_page_query:
                                    DictTypePageQueryModel = Depends(DictTypePageQueryModel.as_query),
    ...
    try:
        # 获取分页数据
        dict_type_page_query_result = DictTypeService.get_dict_type_list_services(query_db, dict_type_page_query,
                                                                                  is_page=True)
        logger.info('获取成功')
...
async def add_system_dict_type(request: Request, add_dict_type: DictTypeModel, query_db: Session = Depends(get_db),
...
async def edit_system_dict_type(request: Request, edit_dict_type: DictTypeModel, query_db: Session = Depends(get_db),
                                current_user: CurrentUserModel = Depends(LoginService.get_current_user)):
    """
    修改字典类型
    """
...
async def query_system_dict_type_options(request: Request, query_db: Session = Depends(get_db)):
    ...
    """
    try:
        dict_type_query_result = DictTypeService.get_dict_type_list_services(query_db, DictTypePageQueryModel(**dict()),
                                                                             is_page=False)
        logger.info(f'获取成功')
...
@dictController.get("/type/{dict_id}", response_model=DictTypeModel,
...
async def export_system_dict_type_list(request: Request,
                                       dict_type_page_query:
                                       DictTypePageQueryModel = Depends(DictTypePageQueryModel.as_form),
                                       query_db: Session = Depends(get_db)):
    """
    导出字典类型信息
    """
    try:
        # 获取全量数据
        dict_type_query_result = DictTypeService.get_dict_type_list_services(query_db, dict_type_page_query,
                                                                             is_page=False)
...
async def query_system_dict_type_data(request: Request, dict_type: str, query_db: Session = Depends(get_db)):
    ...
    try:
        # 获取全量数据
        dict_data_query_result = await DictDataService.query_dict_data_list_from_cache_services(request.app.state.redis,
                                                                                                dict_type)
        logger.info('获取成功')
...
@dictController.get("/data/list", response_model=PageResponseModel,
                    dependencies=[Depends(CheckUserInterfaceAuth('system:dict:list'))])
async def get_system_dict_data_list(request: Request,
                                    dict_data_page_query:
                                    DictDataPageQueryModel = Depends(DictDataPageQueryModel.as_query),
                                    query_db: Session = Depends(get_db)):
    """
    获取字典数据列表分页数据
    """
    try:
        # 获取分页数据
        dict_data_page_query_result = DictDataService.get_dict_data_list_services(query_db, dict_data_page_query,
                                                                                  is_page=True)
        logger.info('获取成功')
...
@log_decorator(title='字典管理', business_type=1)
async def add_system_dict_data(request: Request, add_dict_data: DictDataModel, query_db: Session = Depends(get_db),
                               current_user: CurrentUserModel = Depends(LoginService.get_current_user)):
    """
    新增字典数据
    """
...
@log_decorator(title='字典管理', business_type=2)
async def edit_system_dict_data(request: Request, edit_dict_data: DictDataModel, query_db: Session = Depends(get_db),
                                current_user: CurrentUserModel = Depends(LoginService.get_current_user)):
    """
    修改字典数据
    """
...
@dictController.get("/data/{dict_code}", response_model=DictDataModel,
                    dependencies=[Depends(CheckUserInterfaceAuth('system:dict:query'))])
...
@log_decorator(title='字典管理', business_type=5)
async def export_system_dict_data_list(request: Request,
                                       dict_data_page_query:
                                       DictDataPageQueryModel = Depends(DictDataPageQueryModel.as_form),
                                       query_db: Session = Depends(get_db)):
    """
    导出字典数据信息
    """
    try:
        # 获取全量数据
        dict_data_query_result = DictDataService.get_dict_data_list_services(query_db, dict_data_page_query,
                                                                             is_page=False)
