...
@configController.get("/list", response_model=PageResponseModel,
                      dependencies=[Depends(CheckUserInterfaceAuth('system:config:list'))])
async def get_system_config_list(request: Request,
...
async def add_system_config(request: Request, add_config: ConfigModel, query_db: Session = Depends(get_db),
...
async def edit_system_config(request: Request, edit_config: ConfigModel, query_db: Session = Depends(get_db),
                             current_user: CurrentUserModel = Depends(LoginService.get_current_user)):
    """
    修改参数配置
    """
...
@configController.get("/{config_id}", response_model=ConfigModel,
                      dependencies=[Depends(CheckUserInterfaceAuth('system:config:query'))])
...
async def export_system_config_list(request: Request,
