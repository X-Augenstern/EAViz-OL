...
from module_admin.service.user_service import UserService, UserRolePageQueryModel, CrudUserRoleModel
...
async def get_system_role_dept_tree(request: Request, role_id: int, query_db: Session = Depends(get_db),
                                    data_scope_sql: str = Depends(GetDataScope('SysDept'))):
    """
    获取部门树以及根据角色id获取最底层部门id列表
    """
    try:
...
@roleController.get("/list", response_model=PageResponseModel,
                    dependencies=[Depends(CheckUserInterfaceAuth('system:role:list'))])
async def get_system_role_list(request: Request,
...
async def add_system_role(request: Request, add_role: AddRoleModel, query_db: Session = Depends(get_db),
...
async def edit_system_role(request: Request, edit_role: AddRoleModel, query_db: Session = Depends(get_db),
                           current_user: CurrentUserModel = Depends(LoginService.get_current_user)):
    """
    修改角色
    """
...
async def edit_system_role_datascope(request: Request, role_data_scope: AddRoleModel,
                                     query_db: Session = Depends(get_db),
                                     current_user: CurrentUserModel = Depends(LoginService.get_current_user)):
    """
    修改（授权）角色权限字符串
...
async def delete_system_role(request: Request, role_ids: str, query_db: Session = Depends(get_db),
                             current_user: CurrentUserModel = Depends(LoginService.get_current_user)):
    """
    删除角色
    """
...
@roleController.get("/{role_id}", response_model=RoleModel,
...
async def export_system_role_list(request: Request,
...
@log_decorator(title='角色管理', business_type=2)
async def reset_system_role_status(request: Request, edit_role: AddRoleModel, query_db: Session = Depends(get_db),
                                   current_user: CurrentUserModel = Depends(LoginService.get_current_user)):
    """
    重置角色状态
    """
...
@roleController.get("/authUser/allocatedList", response_model=PageResponseModel,
                    dependencies=[Depends(CheckUserInterfaceAuth('system:role:list'))])
async def get_system_allocated_user_list(request: Request,
    ...
    """
    try:
        role_user_allocated_page_query_result = RoleService.get_role_user_allocated_list_services(query_db, user_role,
                                                                                                  is_page=True)
        logger.info('获取成功')
...
@roleController.get("/authUser/unallocatedList", response_model=PageResponseModel,
                    dependencies=[Depends(CheckUserInterfaceAuth('system:role:list'))])
async def get_system_unallocated_user_list(request: Request,
                                           user_role: UserRolePageQueryModel = Depends(UserRolePageQueryModel.as_query),
                                           query_db: Session = Depends(get_db)):
    """
    获取未分配角色的用户列表
    """
    try:
        role_user_unallocated_page_query_result = RoleService.get_role_user_unallocated_list_services(query_db,
                                                                                                      user_role,
                                                                                                      is_page=True)
        logger.info('获取成功')
...
@log_decorator(title='角色管理', business_type=4)
async def add_system_role_user(request: Request, add_role_user: CrudUserRoleModel = Depends(CrudUserRoleModel.as_query),
                               query_db: Session = Depends(get_db)):
    """
    为用户新增角色
    """
...
@log_decorator(title='角色管理', business_type=4)
async def cancel_system_role_user(request: Request, cancel_user_role: CrudUserRoleModel,
                                  query_db: Session = Depends(get_db)):
    """
    为用户删除角色
    """
...
@log_decorator(title='角色管理', business_type=4)
async def batch_cancel_system_role_user(request: Request,
