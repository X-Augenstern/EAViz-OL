...
@deptController.get("/list/exclude/{dept_id}", response_model=List[DeptModel],
                    dependencies=[Depends(CheckUserInterfaceAuth('system:dept:list'))])
async def get_system_dept_tree_for_edit_option(request: Request, dept_id: int, query_db: Session = Depends(get_db),
...
@deptController.get("/list", response_model=List[DeptModel],
                    dependencies=[Depends(CheckUserInterfaceAuth('system:dept:list'))])
async def get_system_dept_list(request: Request, dept_query: DeptQueryModel = Depends(DeptQueryModel.as_query),
...
async def add_system_dept(request: Request, add_dept: DeptModel, query_db: Session = Depends(get_db),
...
async def edit_system_dept(request: Request, edit_dept: DeptModel, query_db: Session = Depends(get_db),
                           current_user: CurrentUserModel = Depends(LoginService.get_current_user)):
    """
    编辑部门
    """
...
async def delete_system_dept(request: Request, dept_ids: str, query_db: Session = Depends(get_db),
                             current_user: CurrentUserModel = Depends(LoginService.get_current_user)):
    """
    删除部门
    """
...
@deptController.get("/{dept_id}", response_model=DeptModel,
