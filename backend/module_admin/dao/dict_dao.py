...
class DictDataDao:
    ...
    def query_dict_data_list(cls, db: Session, dict_type: str):
        ...
        :return: 字典数据列表信息对象
        """
        # 查询调用的是 db.query(SysDictData)，所以返回的结果集将主要包含 SysDictData 的列数据
        # 如果要在同一个查询中同时获取 SysDictData 和 SysDictType 表的数据，可以通过修改查询对象来包含这两个模型
        # 将 db.query(SysDictData, SysDictType) 作为查询的起点，这将允许 SQLAlchemy 在返回的结果集中包含这两个表的字段
        # 当执行这样的查询时，结果 query 是一个列表，列表中的每个元素都是一个元组。元组中对象的顺序与在 query() 方法中指定的顺序是一致的
        # 当调用 db.query(SysDictData, SysDictType)，返回的元组中，SysDictData 的实例将会是元组的第一个元素，SysDictType 的实例将会是第二个元素
        # 处理查询结果时，可以直接通过解构元组来访问每个实例 for dict_data, dict_type in dict_data_list:
