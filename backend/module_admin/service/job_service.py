...
class JobService:
    ...
    def edit_job_services(cls, query_db: Session, page_object: EditJobModel):
        ...
        :return: 编辑定时任务校验结果
        """
        existing_job_service = cls.job_detail_services(query_db, page_object.job_id)
        if not existing_job_service:
            return CrudResponseModel(is_success=False, message='定时任务不存在')

        conflicting_job_service = JobDao.get_job_detail_by_info(query_db, page_object)
        if conflicting_job_service and conflicting_job_service.job_id != page_object.job_id:
            return CrudResponseModel(is_success=False, message='存在冲突的定时任务')  # 一旦检测到冲突，应当立即停止进一步操作，并向用户反馈错误信息。
    ...
    def execute_job_once_services(cls, query_db: Session, page_object: JobModel):
        ...
        1、从APScheduler中获取任务对象，如果已存在于APScheduler任务调度器中，从中删除此任务对象

        2、从DB中获取任务对象，如果能获取到，执行一次，否则报不存在
    ...
    async def export_job_list_services(request: Request, job_list: List):
        ...
        # [{"label":"默认","value":"default"},{"label":"数据库","value":"sqlalchemy"},{"label":"redis","value":"redis"}]
        job_group_option = [dict(label=item.get('dictLabel'), value=item.get('dictValue')) for item in job_group_list]
        # {"default":{"label":"默认","value":"default"},"sqlalchemy":{"label":"数据库","value":"sqlalchemy"},"redis":{"label":"redis","value":"redis"}}
        ...
        # [{"label":"默认","value":"default"},{"label":"进程池"","value":"processpool"}]
        job_executor_option = [dict(label=item.get('dictLabel'), value=item.get('dictValue')) for item in
                               job_executor_list]
        # {"default":{"label":"默认","value":"default"},{"processpool":{"label":"进程池"","value":"processpool"}}}
