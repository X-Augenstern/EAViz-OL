...
class ConfigService:
    ...
    async def edit_config_services(cls, request: Request, query_db: Session, page_object: ConfigModel):
        ...
        :return: 编辑参数配置校验结果
        """
        existing_config = cls.config_detail_services(query_db, page_object.config_id)
        if not existing_config:
            return CrudResponseModel(is_success=False, message='参数配置不存在')
    ...
    async def delete_config_services(cls, request: Request, query_db: Session, page_object: DeleteConfigModel):
        ...
        if page_object.config_ids:
            ...
            try:
                ...
                query_db.rollback()
                result = dict(is_success=True, message=f'删除失败: {str(e)}')
        else:
