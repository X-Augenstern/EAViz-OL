...
class PostService:
    ...
    def edit_post_services(cls, query_db: Session, page_object: PostModel):
        ...
        :return: 编辑岗位校验结果
        """
        existing_post = cls.post_detail_services(query_db, page_object.post_id)
        if not existing_post:
            return CrudResponseModel(is_success=False, message='岗位不存在')

        conflicting_post = PostDao.get_post_detail_by_info(query_db, PostModel(postName=page_object.post_name))
        if conflicting_post and conflicting_post.post_id != page_object.post_id:
            result = dict(is_success=False, message='岗位名称已存在')
            return CrudResponseModel(**result)
        try:
