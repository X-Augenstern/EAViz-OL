...
class NoticeService:
    ...
    def edit_notice_services(cls, query_db: Session, page_object: NoticeModel):
        ...
        :return: 编辑通知公告校验结果
        """
        existing_notice = cls.notice_detail_services(query_db, page_object.notice_id)
        if not existing_notice:
            return CrudResponseModel(is_success=False, message='通知公告不存在')

        conflicting_notice = NoticeDao.get_notice_detail_by_info(query_db, page_object)
        if conflicting_notice and conflicting_notice.notice_id != page_object.notice_id:
            result = dict(is_success=False, message='与已存在的通知公告发生冲突')
            return CrudResponseModel(**result)
