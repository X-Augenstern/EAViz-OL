  ...
  <div class="app-container">
    ...
    >
      <el-form-item label="EDF名称" prop="edfName">
        <el-input
          v-model="queryParams.edfName"
          placeholder="请输入EDF名称"
          clearable
          style="width: 240px"
      ...
      >
        <template #default="scope">
          <el-tooltip content="绘制EEG" placement="top">
            <el-button
              link
              v-show="!EEGChartStore.isExist"
              type="primary"
              icon="Picture"
              @click="handlePlot(scope.row)"
              v-hasPermi="['system:edf:getData']"
            ></el-button>
          </el-tooltip>
...
const edfListWithCount = computed(() =>
  edfList.value.map((item) => ({
    ...item,
    validChannelsCount: item.validChannels
      ? item.validChannels.split(",").length
      : 0,
  }))
);
...
function getList() {
  ...
  proxy.$refs["queryRef"].validate((valid) => {
    if (valid) {
      loading.value = true;
      listEdf(proxy.addDateRange(queryParams.value, dateRange.value)).then(
        (res) => {
          loading.value = false;
...
function handleDelete(row) {
  ...
  proxy.$modal
    .confirm('是否确认删除EDF编号为"' + edfIds + '"的数据项？')
    .then(function () {
      return delEdf(edfIds);
    })
    .then(() => {
      getList();
      EDFStore.refreshListEDF();
...
const handleFileError = (err, file, fileList) => {
  uploadParams.uploadedFiles++;
  uploadParams.responses.push(
    `${file.name}上传失败：${response.msg} 错误信息：${err}`
  );
